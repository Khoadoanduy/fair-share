...
export default function CollectPaymentScreen() {
  ...
  const fetchPaymentSheetParams = async () => {
    try {
      ...
      await axios.put(`${API_URL}/api/user/${clerkId}`, {
        name: customer,
      });

      // Update Redux state with Stripe customer ID
  ...
  const openPaymentSheet = async () => {
    ...
    if (error) {
      ...
      // Update Redux state to indicate user has a payment method
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.title}>
          {hasPayment ? "Update Payment Method" : "Add Payment Method"}
        </Text>
        <Text style={styles.description}>
          {hasPayment
            ? "Update your payment method for FairShare"
        ...
        {loading ? (
          ...
          <Button
            title={
              hasPayment ? "Update payment method" : "Set up payment method"
            }
